...
class GeneratePortfolioMarkedToMarket: 
    ...
    def portfolio_marked_to_market(self, operations_datatable: DataTable, date:datetime=None)->DataTable:
        operations_limited_by_date = operations_datatable.limit_date(self.__column_mapper.date(), date)
        ...
        for row in result:
            ...
            result.update(row['index'], self.__column_mapper.market_value(), market_value)
        return result
